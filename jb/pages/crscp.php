...
<div class="sub2" style="width:100%;height:100%;margin-top:5%;display:none;">
	<h1 style="margin-left:40%;">Select a Store</h1>
	<div id="stores_list" style="margin-left:40%;">
	<input name="sload" type="radio" style="margin-left:5%;" value="none">Create a New Store</input></br>
	</div>
	<button id="load_store" style="margin-left:45%;">Manage Store</button>
</div>
<div id="main" class="content" style="width:100%;margin-top:5%;display:none;">
<div id="tabs">
    <ul>
        ...
        <li><a href="#tabs-5">Store Inventory</a></li>
...
<!--step 5-->
    ...
    <div id="tabs-5">
        <h2 style="margin-left:20%;">Store Inventory</h2>
    ...
    <div id="tabs-6">
        <h2 style="margin-left:20%;">Publish Store</h2>
show store
    </div>
...
/*/////////////////////////////////////////////
0.load buttons
1.send color and selection and save them
2.a. if the store id is none then have the user select a store
	A.either load the stores for the user or create a new store based on the users responce or do nothing
3.load the store
//////////////////////////////////////////////*/
/*new start*/
alert("new start");
	...
	var store = '<?php echo $store;?>';
	    if(color != "none")
	    {
		var postData0 = {'action': 'start', 'color': color, 'selection': selection, 'store': store};
	    }
	    else
	    {
            	var postData0 = {'action': 'start', 'color': color, 'store': store};
	    }
            $.ajax({
                type: "POST",
                data: postData0,
				...
				if(store == "none")
				{
					...
					$('.content').hide();
					$('.sub2').show();
					//alert("hereiam"+data.stores[1].store_id);
//load data.stores and display in html
...
alert("ids:"+data.stores[i].store_id);
							$('#stores_list').append("<input name='sload' type='radio' style='margin-left:5%;'value='"+data.stores[i].store_id+"'>"+"Store Without a Name"+"</input></br>");
						}
						else
						{
							$('#stores_list').append("<input name='sload' type='radio' style='margin-left:5%;'value='"+data.stores[i].store_id+"'>"+data.stores[i].sname+"</input></br>");
						}
...
alert("load store2"+store2);
				//$('input:radio[name=lfont]:checked').val();
		load_store(store2);
alert("what");
        });
//-------------------------------//////////////////////////////////////////////
...
$('.content').show(); 
		$('.sub2').hide();
	    var postData0 = {'action': 'load_store', 'store_id': store_id};
	    $.ajax({
	        type: "POST",
	        data: postData0,
	        url: "create_store_backend.php",                  //  
